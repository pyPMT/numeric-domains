;; Non-linear extension of the sugar supply-chain domain.
...
;;
;; NLNP extension: Mill processing cost includes an overhead term
;; (* (wear ?m) (has-resource ?r ?m)) — machine wear × batch size.
;; Worn machines cost more when processing large stockpiles, modelling
;; increased maintenance and inefficiency. Wear accumulates with use.

(define (domain nlnp_supply_chain)
	(:types
	...
	(:action produce_sugar
		...
		:effect	     (and
				...
				(increase (mill-cost)(* (wear ?m) (has-resource ?r ?m)))
				(increase (wear ?m) 0.1)
		     	     )
	)
