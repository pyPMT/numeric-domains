;; Non-linear extension of the settlers (settlersnumeric) domain.
;; Original: Introduced by Maria Fox and Derek Long in IPC-3.
;;
;; NLNP extension: Pollution from iron smelting depends on
;; (* (available ore ?p) (available coal ?p)) — the scale of raw materials
;; present at the smelting site. Larger stockpiles of ore and coal produce
;; more pollution per smelting operation due to increased furnace emissions
;; and material handling. This creates a non-linear trade-off between
;; stockpiling efficiency and environmental cost.

(define (domain nlnp_civ)
  ...
  ;; NLNP: pollution from smelting depends on scale of materials present
  (:action make-iron
   :parameters (?p - place)
   ...
   :effect (and (decrease (available ore ?p) 1)
		...
		(increase (pollution) (* (available ore ?p) (available coal ?p)))))
