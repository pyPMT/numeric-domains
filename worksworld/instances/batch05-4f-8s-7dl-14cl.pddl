; problem: wfc4-free-8s-7dl-14cl
; domain: worksworld-simplified
; 8 sites, star topology (1 cloud hub + 7 region peripherals), 7 direct links, 14 composite links, 4 workflow components, linear chain, free goal, multi-composite-link

(define (problem wfc4-free-8s-7dl-14cl)(:domain worksworld-simplified)
  ...
  (:objects
    ...
    ; workflow components
    dc0 dc1 dc2 - data_component
    dp0 dp1 - processing_component

    ; data component types:
    dt0 dt1 dt2 - data_component_type

    ; processing component types:
    pt0 pt1 - processing_component_type
  ...
  (:init
    ...
    ; dc2 - msg metrics
    (= (msg_max_rate dc2) 129) ; max possible msg/s
    
    ; dc2 - work_amounts
    (= (work_amount dc2 config) 0.000595703125) ; config size in MB
    (= (work_amount dc2 storage) 666.66796875) ; storage requirements (MB): 0.001435546875 MB/msg * 129 msg/s * 3600s
    ...
    (type_of dp0 pt0)
    (= (msg_max_rate dp0) 122) ; max possible msg/s
    ...
    (type_of dp1 pt1)
    (= (msg_max_rate dp1) 128) ; max possible msg/s
    ...
    (= (network_latency link-cloud1-region1) 0.023) ; 23ms
    ...
    (= (network_latency link-cloud1-region2) 0.025) ; 24ms
    ...
    (= (network_latency link-cloud1-region3) 0.025) ; 25ms
    ...
    (= (network_latency link-cloud1-region4) 0.033) ; 32ms
    ...
    (= (network_latency link-cloud1-region7) 0.033) ; 32ms
    ...
    (= (network_latency composite-region1-region3) 0.048) ; 48ms (sum of sub-links)
    ...
    (= (network_latency composite-region1-region4) 0.056) ; 56ms (sum of sub-links)
    ...
    (= (network_latency composite-region1-region6) 0.049) ; 48ms (sum of sub-links)
    ...
    (= (network_latency composite-region1-region7) 0.056) ; 55ms (sum of sub-links)
    ...
    (= (network_latency composite-region2-region4) 0.057) ; 57ms (sum of sub-links)
    ...
    (= (network_latency composite-region2-region6) 0.050) ; 50ms (sum of sub-links)
    ...
    (= (network_latency composite-region2-region7) 0.057) ; 57ms (sum of sub-links)
    ...
    (= (network_latency composite-region3-region4) 0.058) ; 57ms (sum of sub-links)
    ...
    (= (network_latency composite-region3-region5) 0.048) ; 47ms (sum of sub-links)
    ...
    (= (network_latency composite-region3-region6) 0.051) ; 50ms (sum of sub-links)
  ...
  (:goal
    ...
    (and
      (exists (?dsi - dsi)
        (and
          (has_data dc2 ?dsi dc0 dsi-region1)
        )
      )
