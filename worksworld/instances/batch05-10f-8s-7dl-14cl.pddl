; problem: wfc10-free-8s-7dl-14cl
; domain: worksworld-simplified
; 8 sites, star topology (1 cloud hub + 7 region peripherals), 7 direct links, 14 composite links, 10 workflow components, linear chain, free goal, multi-composite-link

(define (problem wfc10-free-8s-7dl-14cl)(:domain worksworld-simplified)
  ...
  (:objects
    ...
    ; workflow components
    dc0 dc1 dc2 dc3 dc4 dc5 - data_component
    dp0 dp1 dp2 dp3 dp4 - processing_component

    ; data component types:
    dt0 dt1 dt2 dt3 dt4 dt5 - data_component_type

    ; processing component types:
    pt0 pt1 pt2 pt3 pt4 - processing_component_type
  ...
  (:init
    ...
    ; dc2 - msg metrics
    (= (msg_max_rate dc2) 135) ; max possible msg/s
    
    ; dc2 - work_amounts
    (= (work_amount dc2 config) 0.000595703125) ; config size in MB
    (= (work_amount dc2 storage) 697.67578125) ; storage requirements (MB): 0.001435546875 MB/msg * 135 msg/s * 3600s
    ...
    ; dc3 - msg metrics
    (= (msg_max_rate dc3) 142) ; max possible msg/s
    
    ; dc3 - work_amounts
    (= (work_amount dc3 config) 0.000615234375) ; config size in MB
    (= (work_amount dc3 storage) 708.890625) ; storage requirements (MB): 0.00138671875 MB/msg * 142 msg/s * 3600s
    ...
    ; dc4 - msg metrics
    (= (msg_max_rate dc4) 148) ; max possible msg/s
    
    ; dc4 - work_amounts
    (= (work_amount dc4 config) 0.000390625) ; config size in MB
    (= (work_amount dc4 storage) 650.390625) ; storage requirements (MB): 0.001220703125 MB/msg * 148 msg/s * 3600s
    ...
    ; dc5 - msg metrics
    (= (msg_max_rate dc5) 156) ; max possible msg/s
    
    ; dc5 - work_amounts
    (= (work_amount dc5 config) 0.00056640625) ; config size in MB
    (= (work_amount dc5 storage) 647.15625) ; storage requirements (MB): 0.00115234375 MB/msg * 156 msg/s * 3600s
    ...
    (type_of dp2 pt2)
    (= (msg_max_rate dp2) 140) ; max possible msg/s
    ...
    (= (network_latency link-cloud1-region1) 0.021) ; 21ms
    ...
    (= (network_latency link-cloud1-region2) 0.028) ; 27ms
    ...
    (= (network_latency link-cloud1-region3) 0.031) ; 31ms
    ...
    (= (network_latency link-cloud1-region6) 0.033) ; 32ms
    ...
    (= (network_latency link-cloud1-region7) 0.024) ; 24ms
    ...
    (= (network_latency composite-region1-region2) 0.049) ; 48ms (sum of sub-links)
    ...
    (= (network_latency composite-region1-region3) 0.052) ; 52ms (sum of sub-links)
    ...
    (= (network_latency composite-region1-region5) 0.047) ; 46ms (sum of sub-links)
    ...
    (= (network_latency composite-region1-region6) 0.054) ; 53ms (sum of sub-links)
    ...
    (= (network_latency composite-region1-region7) 0.045) ; 45ms (sum of sub-links)
    ...
    (= (network_latency composite-region2-region3) 0.059) ; 59ms (sum of sub-links)
    ...
    (= (network_latency composite-region2-region5) 0.053) ; 53ms (sum of sub-links)
    ...
    (= (network_latency composite-region2-region6) 0.061) ; 60ms (sum of sub-links)
    ...
    (= (network_latency composite-region2-region7) 0.052) ; 52ms (sum of sub-links)
    ...
    (= (network_latency composite-region3-region4) 0.060) ; 59ms (sum of sub-links)
    ...
    (= (network_latency composite-region3-region6) 0.064) ; 63ms (sum of sub-links)
  ...
  (:goal
    ...
    (and
      (exists (?dsi - dsi)
        (and
          (has_data dc5 ?dsi dc0 dsi-region1)
        )
      )
