; problem: wfc6-free-8s-7dl-14cl
; domain: worksworld-simplified
; 8 sites, star topology (1 cloud hub + 7 region peripherals), 7 direct links, 14 composite links, 6 workflow components, linear chain, free goal, multi-composite-link

(define (problem wfc6-free-8s-7dl-14cl)(:domain worksworld-simplified)
  ...
  (:objects
    ...
    ; workflow components
    dc0 dc1 dc2 dc3 - data_component
    dp0 dp1 dp2 - processing_component

    ; data component types:
    dt0 dt1 dt2 dt3 - data_component_type

    ; processing component types:
    pt0 pt1 pt2 - processing_component_type
  ...
  (:init
    ...
    ; dc2 - msg metrics
    (= (msg_max_rate dc2) 126) ; max possible msg/s
    
    ; dc2 - work_amounts
    (= (work_amount dc2 config) 0.000595703125) ; config size in MB
    (= (work_amount dc2 storage) 651.1640625) ; storage requirements (MB): 0.001435546875 MB/msg * 126 msg/s * 3600s
    ...
    (type_of dp2 pt2)
    (= (msg_max_rate dp2) 129) ; max possible msg/s
    ...
    (= (network_latency link-cloud1-region3) 0.021) ; 20ms
    ...
    (= (network_latency link-cloud1-region5) 0.022) ; 22ms
    ...
    (= (network_latency link-cloud1-region6) 0.024) ; 23ms
    ...
    (= (network_latency link-cloud1-region7) 0.020) ; 20ms
    ...
    (= (network_latency composite-region1-region2) 0.059) ; 59ms (sum of sub-links)
    ...
    (= (network_latency composite-region1-region4) 0.058) ; 57ms (sum of sub-links)
    ...
    (= (network_latency composite-region1-region5) 0.050) ; 49ms (sum of sub-links)
    ...
    (= (network_latency composite-region1-region6) 0.051) ; 51ms (sum of sub-links)
    ...
    (= (network_latency composite-region1-region7) 0.048) ; 47ms (sum of sub-links)
    ...
    (= (network_latency composite-region2-region3) 0.053) ; 52ms (sum of sub-links)
    ...
    (= (network_latency composite-region2-region5) 0.054) ; 53ms (sum of sub-links)
    ...
    (= (network_latency composite-region2-region6) 0.055) ; 55ms (sum of sub-links)
    ...
    (= (network_latency composite-region2-region7) 0.052) ; 51ms (sum of sub-links)
    ...
    (= (network_latency composite-region3-region5) 0.043) ; 43ms (sum of sub-links)
    ...
    (= (network_latency composite-region3-region6) 0.045) ; 44ms (sum of sub-links)
  ...
  (:goal
    ...
    (and
      (exists (?dsi - dsi)
        (and
          (has_data dc3 ?dsi dc0 dsi-region1)
        )
      )
